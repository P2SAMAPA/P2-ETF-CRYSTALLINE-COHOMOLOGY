...
st.set_page_config(page_title="Crystalline Cohomology", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Crystalline Cohomology</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">p-adic cohomology | de Rham-Witt complexes | Microstructure torsion invariants</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Crystalline")
...
st.sidebar.markdown(f"**p-adic base:** {config.P_ADIC_BASE} | **Tick size:** {config.TICK_SIZE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'crystalline_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Torsion invariant: {etf['crystalline_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Torsion"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Torsion", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Crystalline Cohomology Torsion (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Crystalline cohomology** uses p-adic cohomology to extract arithmetic invariants.
        - **de Rham-Witt complexes** capture microstructure effects from discrete price grids.
        - **Torsion information** measures microstructure noise invisible to real-valued analysis.
        - The score is the torsion invariant (p-adic valuations + variance).
        - Higher score → more microstructure structure / noise → potential alpha.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the torsion invariants.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Crystalline Cohomology | p-adic cohomology for microstructure invariants")
